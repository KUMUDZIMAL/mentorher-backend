...
export async function GET(req: NextRequest) {
  ...
  if (receiverId) {
    ...
    // Check follow relationship: allow messaging if current user follows receiver OR vice versa
  ...
  return NextResponse.json({ error: "Missing receiverId or groupId query parameter" }, { status: 400 });
}
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ error: "Missing receiverId or groupId in request body" }, { status: 400 });
}
